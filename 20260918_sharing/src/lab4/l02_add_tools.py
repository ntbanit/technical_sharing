...
def ask_question(order_id: str) :
    messages = [{"role": "user", "content": 
                 f"What's the status of order {order_id}, and what is the return policy for that product?"
                 }]
    ...
    while response.stop_reason == "tool_use":
        ...
        for tool_use_block in response.content:
            ...
            if tool_use_block.name == "get_order_status":
                result = get_order_status(**tool_use_block.input)

            elif tool_use_block.name == "get_return_policy":
                result = get_return_policy(**tool_use_block.input)
